...
CTE_token_in AS (
    SELECT
        CAST("to" AS VARCHAR) AS address,
        SUM(CAST(value AS DOUBLE)) / POWER(10, 18) AS token_in
    FROM erc20_ethereum.evt_Transfer
    WHERE
        CAST("to" AS VARCHAR) IN (SELECT address FROM CTE_holder_address)
        AND contract_address = 0x0000000000085d4780B73119b644AE5ecd22b376
...
CTE_token_out AS (
    SELECT
        CAST("from" AS VARCHAR) AS address,
        SUM(CAST(value AS DOUBLE)) / POWER(10, 18) AS token_out
    FROM erc20_ethereum.evt_Transfer
    WHERE
