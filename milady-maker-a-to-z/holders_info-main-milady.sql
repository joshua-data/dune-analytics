...
-- 완성: 일자별로 NFT를 1개 이상 보유한 address 수, 전일 대비 증감 수, 양/음 여부 구하기
SELECT
    date,
    COUNT(address) FILTER (WHERE running_balance > 0) AS holders_cnt,
    COUNT(address) FILTER (WHERE running_balance > 0)
    - LAG(COUNT(address) FILTER (WHERE running_balance > 0), 1) OVER (ORDER BY date) AS holders_dod,
    COUNT(address) FILTER (WHERE running_balance > 0)
    - LAG(COUNT(address) FILTER (WHERE running_balance > 0), 7) OVER (ORDER BY date) AS holders_wow,
    COUNT(address) FILTER (WHERE running_balance > 0)
    - LAG(COUNT(address) FILTER (WHERE running_balance > 0), 30) OVER (ORDER BY date) AS holders_mom,
    CASE
        WHEN
            COUNT(address) FILTER (WHERE running_balance > 0)
            - LAG(COUNT(address) FILTER (WHERE running_balance > 0), 1) OVER (ORDER BY date) > 0 THEN 'positive'
        ELSE 'negative'
    END AS holders_dod_group
